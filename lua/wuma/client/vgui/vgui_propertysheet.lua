...
function PANEL:Init()
	...
	self.close_button.DoClick = function(button) self:OnCloseButtonPressed() end
	...
	self.close_button.OnCursorEntered = function(button)
		button.Highlight = Color(42, 115, 180)
		button:Paint(button:GetSize())
	end

	self.close_button.OnCursorExited = function(button)
		button.Highlight = Color(59, 59, 59)
		button:Paint(button:GetSize())
	end
...
function PANEL:SetActiveTab(active)
	...
	if self.disregard_first then
		self:OnTabChange(active:GetText())
	else
		self.disregard_first = true
	end
