...
object:AddProperty("data", "Data", setmetatable({__newindex = function(self, k, v) WUMAError("Cannot create new index on WUMA Repository") end}), false)
...
if SERVER then
	function object:Put(key, value)
		...
		for player in self:GetSubscribers() do
			WUMA.HasAccess(player, self:GetAccess(), function(authorized)
				if authorized then
