...
function WUMA.InitializeCommands()	
	WUMA.SetOverride(WUMA.COMMAND,function()
		concommand.Add("wuma", WUMA.Commands.Callback, nil, cmd:GetHelp())
	end,function()
		concommand.Remove("wuma")
	end)
	
	local override = WUMA.GetOverride(WUMA.COMMAND)
	if override then
		override()
	end
end

function WUMA.Commands.Callback(ply,cmd,args) 
	WUMADebug(ply)
	WUMADebug(cmd)
	PrintTable(args)
end

WUMA.Commands.Restrict = WUMACommand:new{cmd="restrict",help="Restrict something from a usergroup."}
WUMA.Commands.Restrict:SetFunction(function(caller, usergroup, type, item, scope)
	usergroup = string.lower(usergroup)
	type = string.lower(type)
	item = string.lower(item)
	scope = scope or false
	
	WUMA.AddRestriction(caller,usergroup,type,item)
end)
WUMA.Commands.Restrict:AddArgument(WUMACommand.PLAYER)
WUMA.Commands.Restrict:AddArgument(WUMACommand.USERGROUP)
WUMA.Commands.Restrict:AddArgument(WUMACommand.STRING,table.GetKeys(Restriction:GetTypes()))
WUMA.Commands.Restrict:AddArgument(WUMACommand.STRING)
WUMA.Commands.Restrict:AddArgument(WUMACommand.SCOPE)

--[[
--Restrict user
local function restrictuser(calling_ply, target_ply, type, item)
	type = string.lower(type)
	item = string.lower(item)
...
restrictuser:addParam{ type=ULib.cmds.StringArg, completes=table.GetKeys(Restriction:GetTypes()), hint="type", error="invalid type \"%s\" specified", ULib.cmds.restrictToCompletes }
...
--Unrestrict
local function unrestrict(calling_ply, usergroup, type, item)
	usergroup = string.lower(usergroup)
	type = string.lower(type)
	item = string.lower(item)
...
unrestrict:addParam{ type=ULib.cmds.StringArg, completes=table.GetKeys(Restriction:GetTypes()), hint="type", error="invalid type \"%s\" specified", ULib.cmds.restrictToCompletes }
...
--Unrestrict user
local function unrestrictuser(calling_ply, target_ply, type, item)
	type = string.lower(type)
	item = string.lower(item)
...
unrestrictuser:addParam{ type=ULib.cmds.StringArg, completes=table.GetKeys(Restriction:GetTypes()), hint="type", error="invalid type \"%s\" specified", ULib.cmds.restrictToCompletes }
...
--Set limit
local function setlimit(calling_ply, usergroup, item, limit)
	usergroup = string.lower(usergroup)
...
--Set user limit
local function setuserlimit(calling_ply, target_ply, item, limit)
	limit = string.lower(limit)
	item = string.lower(item)

	WUMA.AddUserLimit(calling_ply,target_ply, item, limit)
end
...
--Unset limit
local function unsetlimit(calling_ply, usergroup, item)
	usergroup = string.lower(usergroup)
	item = string.lower(item)
...
--Unset user limit
local function unsetuserlimit(calling_ply, target_ply, item)
	item = string.lower(item)
...
--Add group loadout
local function addloadout(calling_ply, usergroup, item, primary, secondary)
	usergroup = string.lower(usergroup)
	item = string.lower(item)
...
--Add user loadout
local function adduserloadout(calling_ply, target_plys, item, primary, secondary)
	item = string.lower(item)
...
local function removeloadout(calling_ply, usergroup, item)
	usergroup = string.lower(usergroup)
	item = string.lower(item)
...
--Delete user loadout
local function removeuserloadout(calling_ply, target_plys, item)
	item = string.lower(item)
...
--Clear group loadout
local function clearloadout(calling_ply, usergroup)
	usergroup = string.lower(usergroup)
...
--Clear user loadout
local function clearuserloadout(calling_ply, target_plys)
...
local function setprimaryweapon(calling_ply, usergroup, item	)
	usergroup = string.lower(usergroup)
...
local function setuserprimaryweapon(calling_ply, users, item	)
